...
enum {
    ...
    SPICE_MAIN_WIDGET_DRAG_START,
    SPICE_MAIN_WIDGET_PATH_GET,
    SPICE_MAIN_XREAD_START,
    SPICE_MAIN_XREAD_STATUS,
    SPICE_MAIN_XREAD_DATA,
...
static const char *agent_msg_types[] = {
    ...
    [ VD_AGENT_DND_WINDOW_REPLY        ] = "dnd window reply",
    [ VD_AGENT_DND_PATH_REPLY          ] = "dnd path reply",
};
...
static void spice_main_channel_class_init(SpiceMainChannelClass *klass)
{
     ...
     * Notify when the mouse mode has changed.
     **/
    signals[SPICE_MAIN_WIDGET_DRAG_START] =
        g_signal_new("widget-drag-start",
                     G_OBJECT_CLASS_TYPE(gobject_class),
                     G_SIGNAL_RUN_FIRST,
                     0,
                     NULL, NULL,
     ...
     * Notify when the mouse mode has changed.
     **/
    signals[SPICE_MAIN_WIDGET_PATH_GET] =
        g_signal_new("widget-path-get",
                     G_OBJECT_CLASS_TYPE(gobject_class),
                     G_SIGNAL_RUN_FIRST,
                     0,
                     NULL, NULL,
     ...
     * Notify when the mouse mode has changed.
     **/
    signals[SPICE_MAIN_XREAD_START] =
        g_signal_new("xread-start",
                     G_OBJECT_CLASS_TYPE(gobject_class),
                     G_SIGNAL_RUN_FIRST,
                     0,
                     NULL, NULL,
     ...
     * Notify when the mouse mode has changed.
     **/
    signals[SPICE_MAIN_XREAD_STATUS] =
        g_signal_new("xread-status",
                     G_OBJECT_CLASS_TYPE(gobject_class),
                     G_SIGNAL_RUN_FIRST,
                     0,
                     NULL, NULL,
     ...
     * Notify when the mouse mode has changed.
     **/
    signals[SPICE_MAIN_XREAD_DATA] =
        g_signal_new("xread-data",
                     G_OBJECT_CLASS_TYPE(gobject_class),
                     G_SIGNAL_RUN_FIRST,
                     0,
                     NULL, NULL,
...
/* coroutine context */
static void main_handle_dnd_window(SpiceMainChannel *channel,
                                   dnd_reply *msg)
{
    if(1 == msg->status) {
        //dnd window generate succeed
        g_coroutine_signal_emit(channel, signals[SPICE_MAIN_WIDGET_DRAG_START], 0,
                                (guint)msg->x, (gchar*)NULL, (guint)msg->y);
    }
}

/* coroutine context */
static void main_handle_dnd_path(SpiceMainChannel *channel,
                                   gchar *msg)
{
    //dnd window generate succeed
    g_coroutine_signal_emit(channel, signals[SPICE_MAIN_WIDGET_PATH_GET], 0,
                            NULL, (gchar*)msg, NULL);
}

/* coroutine context */
static void file_xread_handle_start(SpiceMainChannel *channel,
                                   gchar *msg)
{
    g_coroutine_signal_emit(channel, signals[SPICE_MAIN_XREAD_START], 0,
                            (gchar*)msg);
}

/* coroutine context */
static void file_xread_handle_data(SpiceMainChannel *channel,
                                   gchar *msg, uint32_t size)
{
    g_coroutine_signal_emit(channel, signals[SPICE_MAIN_XREAD_DATA], 0,
                            (gchar*)msg, size);
}

/* coroutine context */
static void file_xread_handle_status(SpiceMainChannel *channel,
                                     gchar *msg)
{
    g_coroutine_signal_emit(channel, signals[SPICE_MAIN_XREAD_STATUS], 0,
                            (gchar*)msg);
}
                                  ...
                                  VDAgentMessage *msg, gpointer payload)
{
    ...
    case VD_AGENT_FILE_XFER_STATUS:
        ...
        break;
    case VD_AGENT_DND_WINDOW_REPLY:
        main_handle_dnd_window(self, payload);
        break;
    case VD_AGENT_DND_PATH_REPLY:
        main_handle_dnd_path(self, payload);
        break;
    case VD_AGENT_FILE_XREAD_START:
        file_xread_handle_start(self, payload);
        break;
    case VD_AGENT_FILE_XREAD_STATUS:
        file_xread_handle_status(self, payload);
        break;
    case VD_AGENT_FILE_XREAD_DATA:
        file_xread_handle_data(self, payload, msg->size);
        break;
    default:
...
gboolean spice_main_xread_request(SpiceMainChannel *channel,
                                  GCancellable *cancellable,
                                  GAsyncReadyCallback callback,
                                  gpointer user_data)
{
    ...
    agent_msg_queue(channel, VD_AGENT_FILE_XREAD_REQUEST,
                    strlen(user_data) + 1, user_data);
...
gboolean spice_main_xread_status(SpiceMainChannel *channel,
                                 GCancellable *cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data,
                                 int size)
{
    ...
    agent_msg_queue(channel, VD_AGENT_FILE_XREAD_STATUS,
                    size, user_data);
...
/**
 * spice_main_dnd_intenal_dst_window:
 * xr: position x where the dst window generated in guest
 * yr: position y where the dst window generated in guest
 */
gboolean spice_main_dnd_intenal_dst_window(SpiceMainChannel *channel,
                                           int xr,
                                           int yr,
                                           GCancellable *cancellable,
                                           GAsyncReadyCallback callback,
                                           gpointer user_data) 
{
    ...
    VDAgentDNDWindowPosition position;
    position.x = xr;
    position.y = yr;

    agent_msg_queue(channel, VD_AGENT_FILE_XREAD_DND_WINDOW,
