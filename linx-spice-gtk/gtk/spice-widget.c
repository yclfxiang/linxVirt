...
typedef struct SpiceFileXReadTask {
    uint32_t                       id;
    int                            file_fd;
    uint64_t                       read_bytes;
    char                           *file_name;
...
static struct vdagent_file_xread *xreads = NULL;
...
static void vdagent_file_xread_task_free(gpointer data);
...
static void drag_data_get_callback(SpiceDisplay *display,
                                   GdkDragContext *drag_context,
                                   GtkSelectionData *data,
                                   guint info,
                                   guint time,
                                   gpointer *user_data)
{
    ...
    gtk_drag_dest_set(GTK_WIDGET(display), GTK_DEST_DEFAULT_ALL,
                      &targets, 1, GDK_ACTION_COPY);

    // The same app won't send DND req before signal 'drag-end' emitted
    if(NULL == gdk_drag_context_get_dest_window(drag_context)) {
        return;
    }
    ...
    spice_main_xread_request(d->main, NULL, NULL, d->file_path->str);

    g_string_free(d->file_path, TRUE);
    d->file_path = NULL;
} 
...
static void widget_path_get(SpiceChannel *channel, guint32 xr, 
                                gchar* temp, guint32 yr, gpointer data)
{
    if(NULL != temp) {
        ...
        d->file_path = g_string_new(temp);

        GtkTargetEntry targets = { "text/uri-list", 0, 0 };
        GtkTargetList *target_list = gtk_target_list_new(&targets, 1);

        gtk_drag_dest_set(GTK_WIDGET(display), GTK_TARGET_OTHER_APP,
                          &targets, 1, GDK_ACTION_COPY);
        gtk_drag_begin(GTK_WIDGET(display), target_list, GDK_ACTION_COPY, 1, NULL);

        gtk_target_list_unref (target_list);
    }
}

static void widget_drag_start(SpiceChannel *channel, guint32 xr, 
                              guint32* temp, guint32 yr, gpointer data)
{
    SpiceDisplay *display = data;
    SpiceDisplayPrivate *d = display->priv;

    spice_inputs_position(d->inputs, xr, yr, get_display_id(display), 1);

    spice_inputs_position(d->inputs, xr, yr, get_display_id(display), 0);
}

/* Parse start message then create a new file xfer task */
static SpiceFileXReadTask *spice_parse_start_msg(
    VDAgentFileXferStartMessage *msg)
{
    GKeyFile *keyfile = NULL;
    SpiceFileXReadTask *task = NULL;
    GError *error = NULL;
    ...
    if (g_key_file_load_from_data(keyfile,
        ...
        g_error("file-xfer: failed to load keyfile: %s",
               error->message);
        goto error;
    }
    task = g_new0(SpiceFileXReadTask, 1);
    task->id = msg->id;
    task->file_name = g_key_file_get_string(
        keyfile, "vdagent-file-xfer", "name", &error);
    if (error) {
        g_error("file-xfer: failed to parse filename: %s",
               error->message);
        goto error;
    }
    ...
    if (error) {
        g_error("file-xfer: failed to parse filesize: %s",
               error->message);
        goto error;
    }
...
error:
    g_clear_error(&error);
    if (task)
        vdagent_file_xread_task_free(task);
    if (keyfile)
        g_key_file_free(keyfile);
    return NULL;
}

static void xread_start(SpiceChannel *channel, VDAgentFileXferStartMessage* msg, 
                        gpointer data)
{
    SpiceFileXReadTask *task;
    VDAgentFileXferStatusMessage status;
    char *dir = NULL, *path = NULL, *file_path = NULL;
    int i;

    if (g_hash_table_lookup(xreads->xread, GUINT_TO_POINTER(msg->id))) {
        g_error("file-xread: error id %u already exists, ignoring!",
               msg->id);
        return;
    }

    task = spice_parse_start_msg(msg);
    if (task == NULL) {
        goto error;
    }

    task->debug = xreads->debug;

    file_path = g_build_filename(xreads->save_dir, task->file_name, NULL);

    dir = g_path_get_dirname(file_path);
    if (g_mkdir_with_parents(dir, 0755) == -1) {
        g_error("file-xread: Failed to create dir %s", dir);
        goto error;
    }

    path = g_strdup(file_path);
    for (i = 0; i < 64 && 
         (g_file_test(path, G_FILE_TEST_EXISTS&G_FILE_TEST_IS_REGULAR) == true);
         i++) {
        g_free(path);
    ...
    task->file_name = path;
    if (i == 64) {
        g_error("file-xread: more then 63 copies of %s exist?",
               file_path);
        goto error;
    }

//    task->file_fd = fopen(path, "wb");
    task->file_fd = g_open (path, O_WRONLY | O_CREAT, 0600);
    if (task->file_fd == -1) {
        g_error("file-xread: failed to create file %s: %s",
               path, strerror(errno));
        goto error;
    }

//    if (ftruncate(task->file_fd, task->file_size) < 0) {
//        g_error("file-xread: err reserving %"PRIu64" bytes for %s: %s",
//               task->file_size, path, strerror(errno));
//        goto error;
//    }

    g_hash_table_insert(xreads->xread, GUINT_TO_POINTER(msg->id), task);

    if (xreads->debug)
        g_debug("file-xread: Adding task %u %s %"PRIu64" bytes",
               task->id, path, task->file_size);

    status.id = msg->id;
    status.result = VD_AGENT_FILE_XREAD_STATUS_CAN_SEND_DATA;
    spice_main_xread_status(xreads->display->priv->main, NULL, NULL,
...
error:
    status.id = msg->id;
    status.result = VD_AGENT_FILE_XREAD_STATUS_ERROR;
    spice_main_xread_status(xreads->display->priv->main, NULL, NULL,
                            &status, sizeof(status));
    if (task)
        vdagent_file_xread_task_free(task);
...
SpiceFileXReadTask *vdagent_file_xread_get_task(uint32_t id)
{
    SpiceFileXReadTask *task;

    task = g_hash_table_lookup(xreads->xread, GUINT_TO_POINTER(id));
    if (task == NULL)
        g_error("file-xfer: error can not find task %u", id);

    return task;
}

static void xread_status(SpiceChannel *channel, VDAgentFileXferStatusMessage* msg, 
                         gpointer data)
{
    SpiceFileXReadTask *task;
    
    task = vdagent_file_xread_get_task(msg->id);
    if (!task)
        return;
    
    switch (msg->result) {
    case VD_AGENT_FILE_XFER_STATUS_CAN_SEND_DATA:
        g_error("file-xfer: task %u %s received unexpected 0 response",
               task->id, task->file_name);
        break;
    default:
        /* Cancel or Error, remove this task */
        g_hash_table_remove(xreads->xread, GUINT_TO_POINTER(msg->id));
    }
}

static void xread_data(SpiceChannel *channel, uint8_t* data_msg, 
                       uint32_t size, gpointer gdata)
{
    if(data_msg == NULL)
        return;
    ...
    VDAgentFileXferDataMessage* msg = (VDAgentFileXferDataMessage*)data_msg;
    char* data = (char*)data_msg + sizeof(VDAgentFileXferDataMessage);

    task = vdagent_file_xread_get_task(msg->id);
    if (!task) 
        return;

    len = write(task->file_fd, data, msg->size);
    if (len == msg->size) {
        ...
        if (task->read_bytes >= task->file_size) {
            if (task->read_bytes == task->file_size) {
                ...
                close(task->file_fd);
                task->file_fd = -1; 
                if (xreads->open_save_dir &&
                        task->file_xfer_nr == task->file_xfer_total) {
#ifdef G_OS_UNIX
                    char buf[PATH_MAX];
                    snprintf(buf, PATH_MAX, "xdg-open '%s'&", xreads->save_dir);
                    status = system(buf);
#endif
                }
                status = VD_AGENT_FILE_XREAD_STATUS_SUCCESS;
            } else {
                g_error("file-xfer: error received too much data");
                status = VD_AGENT_FILE_XREAD_STATUS_ERROR;
            }
        }
    } else {
        g_error("file-xfer: error writing %s: %s", task->file_name,
               strerror(errno));
        status = VD_AGENT_FILE_XREAD_STATUS_ERROR;
    }  

    if (status != -1) {
        VDAgentFileXferStatusMessage statusMsg;
        statusMsg.id = msg->id;
        statusMsg.result = status;
        spice_main_xread_status(xreads->display->priv->main, NULL, NULL,
                                &statusMsg, sizeof(statusMsg));
        g_hash_table_remove(xreads->xread, GUINT_TO_POINTER(msg->id));
    }
}
...
static void spice_display_init(SpiceDisplay *display)
{
    ...
    g_signal_connect(display, "drag-end", G_CALLBACK (drag_data_get_callback), NULL);
...
static gboolean motion_event(GtkWidget *widget, GdkEventMotion *motion)
{
    ...
    if(d->grab_status) {
        if(g_get_real_time() - d->grab_time > (1<<20)) {
            return true;
        } else {
            d->grab_status = false;
        }
    ...
    d->mouse_record_state = motion->state;
    switch (d->mouse_mode) {
    case SPICE_MOUSE_MODE_CLIENT:
#ifdef GDK_WINDOWING_X11
        if(gdk_pointer_is_grabbed()) {
            if(!d->wait_drop_status) {
                if((x == 0 || y == 0 || x == d->area.width - 1
                    || y == d->area.height - 1) && (motion->state & GDK_BUTTON1_MASK)) {
                    d->grab_time = g_get_real_time();
                    d->mouse_lastdown_x = x;
                    d->mouse_lastdown_y = y;
                    d->wait_drop_status = true;

                    spice_main_dnd_intenal_dst_window(d->main, x, y, NULL, NULL, NULL);
                }
            } else {
                if(g_get_real_time() - d->grab_time < (1<<20)) {
                    //stop the pointer
                    ...
                    gdk_display_warp_pointer(gtk_widget_get_display(GTK_WIDGET(display)),
                                             screen, wx + d->mouse_lastdown_x,
                                             wy + d->mouse_lastdown_y);
                }
            }
        }
#endif
...
static gboolean button_event(GtkWidget *widget, GdkEventButton *button)
{
    ...
    {
        if(button->type == GDK_BUTTON_PRESS && !gdk_pointer_is_grabbed()) {
            GdkWindow *window = GDK_WINDOW(gtk_widget_get_window(widget));

            //GTK3 do not support param xconfine_to
            GdkGrabStatus status = gdk_pointer_grab(window,FALSE,
                                                    GDK_POINTER_MOTION_MASK |
                                                    GDK_BUTTON_PRESS_MASK |
                                                    GDK_BUTTON_RELEASE_MASK |
                                                    GDK_BUTTON_MOTION_MASK |
            ...
            if (status != GrabSuccess) {
                printf("pointer grab failed %d\n", status);
            }
        } else if(button->type == GDK_BUTTON_RELEASE && gdk_pointer_is_grabbed()) {
            gdk_pointer_ungrab(GDK_CURRENT_TIME);
            d->wait_drop_status = false;
        }
    }
#endif
...
static void vdagent_file_xread_task_free(gpointer data)
{
    SpiceFileXReadTask *task = data;

    g_return_if_fail(task != NULL);

    if (task->file_fd != -1) {
        g_error("file-xread: Removing task %u and file %s due to error",
               task->id, task->file_name);
        close(task->file_fd);
        remove(task->file_name);
    } else {
...
static void vdagent_file_xfers_destroy(struct vdagent_file_xread *xread)
{
    if(!xread)
        return ;

    g_hash_table_destroy(xread->xread); 
    g_free(xread->save_dir);
    g_free(xread);
}  

static struct vdagent_file_xread *vdagent_file_xread_create(
    SpiceDisplay *display, const char *save_dir,
    int open_save_dir, int debug)
{
    struct vdagent_file_xread *xreads;

    xreads = g_malloc(sizeof(*xreads));
    xreads->xread = g_hash_table_new_full(g_direct_hash, g_direct_equal,
                                         NULL, vdagent_file_xread_task_free);
    xreads->display = display;
    xreads->save_dir = g_strdup(save_dir);
    xreads->open_save_dir = open_save_dir;
    xreads->debug = debug;
...
static void channel_new(SpiceSession *s, SpiceChannel *channel, gpointer data)
{
    ...
    if (SPICE_IS_MAIN_CHANNEL(channel)) {
        ...
        g_signal_connect(channel, "widget-drag-start",
                                      G_CALLBACK(widget_drag_start), display);
        g_signal_connect(channel, "widget-path-get",
                                      G_CALLBACK(widget_path_get), display);
        g_signal_connect(channel, "xread-start",
                                      G_CALLBACK(xread_start), NULL);
        g_signal_connect(channel, "xread-status",
                                      G_CALLBACK(xread_status), NULL);
        g_signal_connect(channel, "xread-data",
                                      G_CALLBACK(xread_data), NULL);
        update_mouse_mode(channel, display);

        const char* fx_dir = g_get_user_special_dir(G_USER_DIRECTORY_DOWNLOAD);
        if(!fx_dir) {
            g_warning("warning could not get file xfer save dir, using cwd");
            fx_dir = ".";
        }

        xreads = vdagent_file_xread_create(display, fx_dir, -1, 0);
...
static void channel_destroy(SpiceSession *s, SpiceChannel *channel, gpointer data)
{
    ...
    if (SPICE_IS_MAIN_CHANNEL(channel)) {
        ...
        vdagent_file_xfers_destroy(xreads);
        xreads = NULL;
        return;
    }
...
{
    SpiceDisplay* spiceDisplay = g_object_new(SPICE_TYPE_DISPLAY, "session", session,
                                              "channel-id", id, NULL);

    if(NULL == spiceDisplay) {
        return spiceDisplay;
    }

    spiceDisplay->priv->wait_drop_status = false;
    spiceDisplay->priv->grab_status = false;
    spiceDisplay->priv->grab_time = 0;
    spiceDisplay->priv->file_path = NULL;
...
{
    SpiceDisplay* spiceDisplay = g_object_new(SPICE_TYPE_DISPLAY,
                                              "session", session,
